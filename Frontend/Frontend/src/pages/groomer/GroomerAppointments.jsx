...
import { getAppointmentsByGroomerId, updateAppointmentStatus } from "../../api-request/groomer-request";
import { useForm } from "react-hook-form";
import Swal from "sweetalert2";
import { Form, Modal, Table } from "react-bootstrap";

export default function GroomerAppointments() {
  const groomerId = useSelector((state) => state.user.groomerId);
  ...
  const fetchAppointments = async () => {
    try {
      const response = await getAppointmentsByGroomerId(groomerId);
      if (response.status) {
