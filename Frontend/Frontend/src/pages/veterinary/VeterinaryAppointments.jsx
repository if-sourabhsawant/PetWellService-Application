...
import { getAppointmentsByVeterinaryUserId, updateAppointmentStatus } from "../../api-request/veterinary-request";
import { useForm } from "react-hook-form";
import Swal from "sweetalert2";
import { Form, Modal, Table } from "react-bootstrap";

export default function VeterinaryAppointments() {
  const veterinaryId = useSelector((state) => state.user.veterinaryId);
  ...
  const fetchAppointments = async () => {
    try {
      const response = await getAppointmentsByVeterinaryUserId(veterinaryId);
      if (response.status) {
